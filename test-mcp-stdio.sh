...
#
# Exercises every tool over the MCP protocol via stdin/stdout (no auth proxy).
# Companion to test-mcp.sh, which tests via HTTP with bearer token auth.
#
...
#
# Usage:
#   ./test-mcp-stdio.sh                    # ephemeral container (dns-mcp-mcp image)
