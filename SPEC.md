...
> **Note (2026-03-01):** This spec was written for the HTTP Streamable transport
> deployment. The `main` branch has since been simplified to stdio-only (single
> container, no proxy, no auth token). The HTTP transport code lives in the
> `remote` branch. This spec remains as the authoritative record for the full
> implementation but does not reflect the current `main` branch architecture.

---

## Architecture (main branch — stdio)
...
## Architecture (remote branch — HTTP Streamable)

```
Client (Claude Desktop / CLI / curl)
  |
  v
...
### Key Architectural Decisions (original HTTP deployment)
