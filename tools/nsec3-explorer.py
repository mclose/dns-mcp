...
def nsec3_hash(name, algorithm=1, iterations=0, salt=b""):
    ...
    if algorithm != 1:
        raise ValueError(
            f"Unsupported NSEC3 algorithm: {algorithm} (only SHA-1/1 is defined)"
        )
...
def main():
    ...
    parser.add_argument(
        "domain", help="Zone to explore (e.g., nsec3-weak.deflationhollow.net)"
    )
    parser.add_argument(
        "--ns", help="Nameserver to query (default: auto-resolve from NS records)"
    )
    parser.add_argument(
        "--probe", help="Specific name to probe (default: _nsec3-probe.<domain>)"
    )
    ...
    if params:
        print(
            f"NSEC3PARAM: algorithm={params['algorithm']} (SHA-1) "
    ...
    for name, types in zone_names.items():
        ...
        if len(label) == 32 and all(
            c in "0123456789ABCDEFGHIJKLMNOPQRSTUVabcdefghijklmnopqrstuv" for c in label
        ):
            continue
...
def show_nsec3_analysis(domain, params, real_names, nsec3_records, probe_name):
    ...
    print("=" * 70)
    print()
    print("  H(name) = SHA-1( wire_format(lowercase(name)) || salt )")
    if iterations > 0:
    ...
    for i, (h, name) in enumerate(entries):
        ...
        print(f"    Next:  {next_h}")
        if i < len(entries) - 1:
            print('    "Nothing exists between me and the next hash."')
        else:
            print('    "Nothing exists between me and the first hash (wrap-around)."')
        print()
    ...
    print(
        f"  Hash chain reveals: {len(entries)} names exist (but not what they're called)"
    )
    print("  Type bitmaps reveal: record types at each hashed name")
    if not salt and iterations == 0:
        print(
            "  Risk: Single-pass SHA-1 with no salt — precomputed dictionary attack is trivial"
        )
        print("         (RFC 9276 accepts this trade-off for resolver performance)")
    elif iterations == 0:
        print(
            f"  Risk: Single-pass SHA-1 with {len(salt)}-byte salt — rainbow tables defeated"
        )
        print("         but per-zone dictionary attack still feasible")
    else:
        print(f"  Risk: {iterations} iterations with salt — slower to brute-force")
        print("         but GPU attacks still practical for common hostnames")
    print()
...
def show_nsec_analysis(domain, real_names, nsec_records, probe_name):
    ...
    print()
    print(
        f"  Zone has {len(real_names)} names — ALL exposed in cleartext via NSEC chain"
    )
    print(
        "  No AXFR needed: just follow NSEC records from the apex to enumerate everything"
    )
    print("  This is the primary reason NSEC3 was created (RFC 5155)")
    print()
