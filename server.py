...
def dns_query(
    ...
    record_type: Literal["A", "AAAA", "MX", "TXT", "NS", "SOA", "CNAME", "PTR", "SRV"] = "A",
    nameserver: str | None = Field(default=None, description="Optional nameserver IP (e.g., '8.8.8.8')")
) -> dict:
    """
...
def timestamp_converter(
    ...
    convert_to: Literal["iso", "epoch", "human"] = "iso"
) -> dict:
    """
...
def reverse_dns(
    ...
    try:
        ...
        return {"error": f"Reverse DNS lookup failed: {str(e)}", "ip_address": ip_address}


@mcp.tool()
def dns_dnssec_validate(
    domain: str = Field(description="Domain name to validate (e.g., 'claude.lab.deflationhollow.net')"),
    record_type: Literal["A", "AAAA", "MX", "TXT", "NS", "SOA", "CNAME"] = "A",
    nameserver: str = Field(default="8.8.8.8", description="DNSSEC-validating resolver to use")
) -> dict:
    """
    ...
    try:
        ...
        try:
            ...
            if root_dnskeys:
                ...
                root_step["key_ids"] = [
                    dns.dnssec.key_id(k) for k in root_dnskeys
                ]
        ...
        parent_dnskeys = root_dnskeys if 'root_dnskeys' in dir() and root_dnskeys else None
        ...
        for i, zone in enumerate(zones[1:], 1):  # Skip root, already done
            ...
            try:
                ...
                if ds_rrset:
                    step["ds_records"] = [
                        {"key_tag": ds.key_tag, "algorithm": ds.algorithm, "digest_type": ds.digest_type}
                        for ds in ds_rrset
                    ]
                    ...
                    if parent_dnskeys and ds_rrsig:
                        try:
                            dns.dnssec.validate(ds_rrset, ds_rrsig, {zones[i-1]: parent_dnskeys})
                            step["validations"].append({
                                "action": "verify DS rdataset",
                                "keyid": ds_rrsig[0].key_tag if ds_rrsig else "unknown",
                                "result": "success"
                            })
                ...
                if zone_dnskeys:
                    ...
                    if ds_rrset and dnskey_rrsig:
                        try:
                            dns.dnssec.validate(zone_dnskeys, dnskey_rrsig, {zone: zone_dnskeys})
                            step["validations"].append({
                                "action": "verify DNSKEY rdataset",
                                "keyid": dnskey_rrsig[0].key_tag if dnskey_rrsig else "unknown",
                                "result": "success"
                            })
                            step["status"] = "secure"
                        except dns.dnssec.ValidationFailure as e:
        ...
        target_step = {"target": domain, "record_type": record_type, "containing_zone": str(target_name.parent()), "validations": []}
            ...
            if target_rrset:
                ...
                if target_rrsig and parent_dnskeys:
                    ...
                    try:
                        ...
                        dns.dnssec.validate(target_rrset, target_rrsig, {signer_name: parent_dnskeys})
                        target_step["validations"].append({
...
@mcp.tool()
def check_spf(
    domain: str = Field(description="Domain to check SPF record for (e.g., 'example.com')")
) -> dict:
    """
    ...
    def _resolve_spf(target: str, depth: int):
        ...
        for part in parts[1:]:  # Skip "v=spf1"
            ...
            if mechanism.startswith("ip4:"):
                network = mechanism[4:]
                mechanisms.append({"type": "ip4", "value": network, "qualifier": qualifier})
                ...
                network = mechanism[4:]
                mechanisms.append({"type": "ip6", "value": network, "qualifier": qualifier})
                if "/" not in network:
                    network += "/128"
                authorized_networks.append(network)
                ...
                mechanisms.append({"type": "include", "value": include_domain, "qualifier": qualifier})
                lookup_count += 1
                _resolve_spf(include_domain, depth + 1)

            elif mechanism.startswith("redirect="):
                redirect_domain = mechanism[9:]
                mechanisms.append({"type": "redirect", "value": redirect_domain, "qualifier": qualifier})
                lookup_count += 1
                ...
                mechanisms.append({"type": "a", "value": a_domain, "qualifier": qualifier})
                lookup_count += 1
                try:
                    resolver = dns.resolver.Resolver()
                ...
                mechanisms.append({"type": "mx", "value": mx_domain, "qualifier": qualifier})
                lookup_count += 1
                try:
                    resolver = dns.resolver.Resolver()
                ...
                mechanisms.append({"type": "exists", "value": mechanism[7:], "qualifier": qualifier})
                lookup_count += 1
...
def check_dmarc(
    ...
    return {
        ...
        "dkim_alignment": alignment_map.get(tags.get("adkim", "r"), tags.get("adkim", "r")),
        "spf_alignment": alignment_map.get(tags.get("aspf", "r"), tags.get("aspf", "r")),
...
def check_dkim_selector(
    selector: str = Field(description="DKIM selector (s= value from DKIM-Signature header)"),
    domain: str = Field(description="DKIM domain (d= value from DKIM-Signature header)")
) -> dict:
    """
...
def check_bimi(
    ...
    selector: str = Field(default="default", description="BIMI selector (almost always 'default')")
) -> dict:
    """
...
def check_mta_sts(
    ...
    fetch_policy: bool = Field(default=True, description="Also fetch the HTTPS policy file from /.well-known/mta-sts.txt")
) -> dict:
    """
...
@mcp.tool()
def check_dane(
    domain: str = Field(description="Domain to check DANE for (the From: domain, not the MX hostname)")
) -> dict:
    """
    ...
    try:
        ...
        mx_list = sorted(
            [(rdata.preference, str(rdata.exchange).rstrip(".")) for rdata in mx_answers],
            key=lambda x: x[0],
        )
    except dns.resolver.NXDOMAIN:
        return {
    ...
    for priority, hostname in mx_list:
        ...
        try:
            ...
            query = dns.message.make_query(tlsa_fqdn, dns.rdatatype.TLSA, want_dnssec=True)
            ...
            for rrset in response.answer:
                if rrset.rdtype == dns.rdatatype.TLSA:
                    for rdata in rrset:
                        tlsa_records.append({
                            "usage": rdata.usage,
                            "usage_name": TLSA_USAGE_NAMES.get(rdata.usage, f"Unknown({rdata.usage})"),
                            "selector": rdata.selector,
                            "selector_name": TLSA_SELECTOR_NAMES.get(rdata.selector, f"Unknown({rdata.selector})"),
                            "matching_type": rdata.mtype,
                            "matching_type_name": TLSA_MATCHING_NAMES.get(rdata.mtype, f"Unknown({rdata.mtype})"),
    ...
    dane_no_dnssec_count = sum(1 for h in mx_hosts if h["dane_status"] == "dane_present_no_dnssec")
    ...
    if dane_valid_count > 0:
        summary_parts.append(f"{dane_valid_count} of {total_mx} MX hosts have valid DANE (TLSA + DNSSEC)")
    ...
    if no_dane_count > 0:
        summary_parts.append(f"{no_dane_count} MX host(s) have no TLSA records published")
    if not mx_hosts:
...
def nsec_info(
    ...
    for rrset in response.authority:
        if rrset.rdtype == dns.rdatatype.NSEC:
            for rdata in rrset:
                ...
                base_result["nsec_records"].append(f"{rrset.name} {rrset.ttl} IN NSEC {rdata}")
        elif rrset.rdtype == dns.rdatatype.NSEC3:
            for rdata in rrset:
                nsec3_records.append(rdata)
                base_result["nsec_records"].append(f"{rrset.name} {rrset.ttl} IN NSEC3 {rdata}")
    ...
    try:
        nsec3param_query = dns.message.make_query(domain, dns.rdatatype.NSEC3PARAM, want_dnssec=True)
    ...
    if base_result["denial_type"] == "nsec3" and nsec3param is not None and nsec3_params is not None:
        mismatches = []
    ...
    elif base_result["denial_type"] == "nsec3" and nsec3param is None and nsec3_params is not None:
        base_result["nsec3_consistency"] = {
            "consistent": False,
            "mismatches": ["No NSEC3PARAM record at zone apex despite NSEC3 denial records"],
        }
...
@mcp.tool()
def rdap_lookup(
    domain: str = Field(description="Domain to look up via RDAP (registrable domain, not subdomain)")
) -> dict:
    """
...
if __name__ == "__main__":
    ...
    if "--stdio" in sys.argv:
        ...
        _print_startup_banner("streamable-http")
