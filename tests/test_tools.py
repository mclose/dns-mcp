...
class TestDnsQuery:
    ...
    def test_nxdomain(self):
        result = dns_query("this-domain-does-not-exist-xyzzy.invalid", "A", nameserver=None)
...
class TestDnsDigStyle:
    ...
    def test_nxdomain(self):
        result = dns_dig_style("this-domain-does-not-exist-xyzzy.invalid", "A", nameserver="8.8.8.8")
...
class TestNsecInfo:
    ...
    def test_response_structure(self):
        ...
        expected_keys = [
            "timestamp", "domain", "zone", "authoritative_ns",
            "authoritative_ip", "probe_name", "response_code",
            "denial_type", "nsec3_params", "nsec_records",
            "zone_walkable", "walkability_risk", "walkability_note",
            "nsec3param", "nsec3_consistency", "errors",
        ]
    ...
    def test_nsec3_consistency_match(self):
        ...
        assert result["nsec3param"]["iterations"] == result["nsec3_params"]["iterations"]
...
class TestDnssecValidate:
    ...
    def test_nxdomain(self):
        result = dns_dnssec_validate("this-domain-does-not-exist-xyzzy.invalid", "A", nameserver="8.8.8.8")
...
class TestCheckSpf:
    ...
    def test_response_structure(self):
        ...
        result = check_spf("google.com")
        for key in ["timestamp", "domain", "raw_record", "mechanisms",
                     "authorized_networks", "all_qualifier", "lookup_count", "errors"]:
            assert key in result, f"Missing key: {key}"
...
class TestCheckDmarc:
    ...
    def test_response_structure(self):
        result = check_dmarc("google.com")
        for key in ["timestamp", "domain", "record_found_at", "raw_record",
                     "policy", "subdomain_policy", "dkim_alignment", "spf_alignment",
                     "percentage", "rua", "ruf", "errors"]:
            assert key in result, f"Missing key: {key}"
...
class TestCheckDkimSelector:
    ...
    def test_nxdomain(self):
        result = check_dkim_selector("selector1", "this-domain-does-not-exist-xyzzy.invalid")
        assert result["record_exists"] is False
        assert len(result["errors"]) > 0
    ...
    def test_response_structure_found(self):
        ...
        for key in ["timestamp", "selector", "domain", "fqdn",
                     "record_exists", "raw_record", "key_type",
                     "key_present", "key_revoked", "flags", "errors"]:
            assert key in result, f"Missing key: {key}"
...
class TestCheckBimi:
    ...
    def test_nxdomain(self):
        result = check_bimi("this-domain-does-not-exist-xyzzy.invalid", selector="default")
        assert result["record_exists"] is False
        assert len(result["errors"]) > 0
    ...
    def test_response_structure(self):
        result = check_bimi("example.com", selector="default")
        for key in ["timestamp", "domain", "selector", "fqdn",
                     "record_exists", "raw_record", "logo_url",
                     "vmc_url", "has_vmc", "errors"]:
            assert key in result, f"Missing key: {key}"
...
class TestCheckMtaSts:
    ...
    def test_nxdomain(self):
        result = check_mta_sts("this-domain-does-not-exist-xyzzy.invalid", fetch_policy=False)
        assert result["record_exists"] is False
        assert len(result["errors"]) > 0
    ...
    def test_response_structure(self):
        ...
        for key in ["timestamp", "domain", "fqdn", "record_exists",
                     "raw_record", "version", "policy_id", "policy", "errors"]:
            assert key in result, f"Missing key: {key}"
...
class TestCheckSmtpTlsrpt:
    ...
    def test_response_structure(self):
        result = check_smtp_tlsrpt("google.com")
        for key in ["timestamp", "domain", "fqdn", "record_exists",
                     "raw_record", "version", "reporting_uris", "errors"]:
            assert key in result, f"Missing key: {key}"
...
class TestRdapLookup:
    ...
    def test_response_structure(self):
        result = rdap_lookup("google.com")
        for key in ["timestamp", "domain", "rdap_server", "registrar",
                     "creation_date", "expiration_date", "last_updated",
                     "status", "registrant_org", "registrant_country",
                     "domain_age_days", "errors"]:
            assert key in result, f"Missing key: {key}"
...
class TestCheckDane:
    ...
    def test_response_structure(self):
        ...
        result = check_dane("google.com")
        for key in ["timestamp", "domain", "mx_hosts", "dane_viable", "summary", "errors"]:
            assert key in result, f"Missing key: {key}"
    ...
    def test_mx_host_structure(self):
        ...
        for key in ["hostname", "priority", "tlsa_fqdn", "has_tlsa",
                     "dnssec_valid", "dane_status", "tlsa_records"]:
